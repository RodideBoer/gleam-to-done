...
pub fn fetch_task(id: Int) -> Promise(Result(Task, ApiError)) {
  let path = "/api/task/" <> int.to_string(id)
  path
...
pub fn patch_task(task: Task) -> Promise(Result(Task, ApiError)) {
  let body =
    ...
    |> task.task_input_to_json
    |> json.to_string
  let path = "/api/task/" <> int.to_string(task.id)
  path
  |> api.patch(task.task_decoder(), json: body)
}

pub fn delete_task(id: Int) -> Promise(Result(Nil, ApiError)) {
  let path = "/api/task/" <> int.to_string(id)
  path
  |> api.delete
}
